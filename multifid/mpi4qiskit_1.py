...
qc.draw(output='mpl', filename=str(sys.argv[2])+'/'+str(sys.argv[1])+'/'+fullenc+'_circ.pdf')
...
plot_histogram(probs, title="processor {}, rank {:d} out of {:d} processors".format(procname, rank, size), filename=str(sys.argv[2])+'/'+str(sys.argv[1])+'/'+fullenc+'_hist.pdf')
...
plt.savefig(str(sys.argv[2])+'/'+str(sys.argv[1])+'/'+fullenc+'_expecs_'+str(sys.argv[3])+'.pdf')
...
np.save(str(sys.argv[2])+'/'+str(sys.argv[1])+'/weights_'+str(rank)+'_'+str(sys.argv[3])+'.npy', np.array([np.dot(weights, vals)]))